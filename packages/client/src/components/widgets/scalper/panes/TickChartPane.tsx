import React, { useCallback } from 'react';
import type { PriceAxis } from '../priceAxis';
import { priceToY, visibleRows, yToPrice } from '../priceAxis';
import type { TickCandle } from '../scalperModel';
import { formatPrice, isBullish } from '../scalperModel';
import * as t from '../scalperTheme';
import { useCanvasRenderer } from '../useCanvas';

/**
 * Tick-chart pane — candles built from a fixed number of prints, drawn on the
 * shared price axis so their bodies line up with the ladder's rows.
 *
 * Ported from scalper-iced (`src/widget/tick_chart_canvas.rs`, Unlicense).
 */

const LABEL_WIDTH = 60;

export interface TickChartPaneProps {
  candles: TickCandle[];
  axis: PriceAxis;
  onWheel?: (e: React.WheelEvent) => void;
}

export const TickChartPane: React.FC<TickChartPaneProps> = ({ candles, axis, onWheel }) => {
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ctx.fillStyle = t.PANEL_BG;
      ctx.fillRect(0, 0, width, height);
      ctx.font = `9px ${t.MONO_FONT}`;
      ctx.textBaseline = 'middle';

      if (!candles.length) {
        ctx.fillStyle = t.TEXT_DIM;
        ctx.textAlign = 'center';
        ctx.fillText('Ticks…', width / 2, height / 2);
        return;
      }

      const rowH = axis.rowHeight;
      const chartWidth = width - LABEL_WIDTH;

      const rows = visibleRows(axis, height);
      const gridEvery = Math.max(1, Math.floor(rows / 8));
      ctx.strokeStyle = t.GRID_LINE;
      ctx.lineWidth = 0.5;
      ctx.fillStyle = t.TEXT_DIM;
      ctx.textAlign = 'right';
      for (let row = -Math.floor(rows / 2); row <= Math.floor(rows / 2); row += gridEvery) {
        const y = height / 2 + row * rowH;
        if (y <= 10 || y >= height - 10) continue;
        ctx.beginPath();
        ctx.moveTo(0, Math.round(y) + 0.5);
        ctx.lineTo(chartWidth, Math.round(y) + 0.5);
        ctx.stroke();
        ctx.fillText(formatPrice(yToPrice(axis, y, height)), width - 3, y);
      }

      const candleWidth = Math.max(3, Math.min(20, chartWidth / candles.length));
      const bodyWidth = Math.max(2, candleWidth * 0.7);

      candles.forEach((candle, i) => {
        const x = chartWidth - (candles.length - i) * candleWidth;
        const centerX = x + candleWidth / 2;
        const openY = priceToY(axis, candle.open, height);
        const closeY = priceToY(axis, candle.close, height);
        const color = isBullish(candle) ? t.BID_GREEN : t.ASK_RED;

        ctx.strokeStyle = color;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(Math.round(centerX) + 0.5, priceToY(axis, candle.high, height));
        ctx.lineTo(Math.round(centerX) + 0.5, priceToY(axis, candle.low, height));
        ctx.stroke();

        // A doji would otherwise be invisible, hence the 2px floor.
        ctx.fillStyle = color;
        ctx.fillRect(
          centerX - bodyWidth / 2,
          Math.min(openY, closeY),
          bodyWidth,
          Math.max(2, Math.abs(openY - closeY)),
        );
      });

      ctx.strokeStyle = t.GRID_LINE;
      ctx.beginPath();
      ctx.moveTo(Math.round(chartWidth) + 0.5, 0);
      ctx.lineTo(Math.round(chartWidth) + 0.5, height);
      ctx.stroke();

      if (axis.lastPrice) {
        const y = priceToY(axis, axis.lastPrice, height) + rowH / 2;
        ctx.strokeStyle = t.LAST_PRICE_LINE;
        ctx.setLineDash([4, 3]);
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(chartWidth, y);
        ctx.stroke();
        ctx.setLineDash([]);
      }
    },
    [candles, axis],
  );

  const { canvasRef, containerRef } = useCanvasRenderer(draw, [candles, axis]);

  return (
    <div ref={containerRef} className="w-full h-full" onWheel={onWheel}>
      <canvas ref={canvasRef} className="block" />
    </div>
  );
};

export default TickChartPane;
