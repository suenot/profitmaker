 ...
 * scripts/curl; skipped entirely when no API_TOKEN is configured) → local
 * session token → SSO JWT from auth.marketmaker.cc (verified against its public
 * JWKS, auto-provisioning a per-user account on first login). Each path yields a
 * user row, so all user-scoped routes work unchanged regardless of how the
...
export const getUserFromRequest = async (request: Request): Promise<AuthUser | null> => {
  ...
  if (matchesApiToken(token)) return await getBootstrapUser();
